...
/**
 * Собрано из множества различных подборок переводов сообщений об ошибках валидаторов Zend.
 */
return [
    // Zend_Validate_Alnum
    ...
    "Luhn algorithm (mod-10 checksum) failed on '%value%'" => "Алгоритм Луна (вычисление контрольной цифры) вернул ошибку для '%value%'",
    "'%value%' seems to contain an invalid checksum" => "Ошибка вычисления контрольной суммы для '%value%'",
    ...
    "'%value%' seems to be an invalid creditcard number" => "'%value%' не является верным номером кредитной карточки",
    ...
    "An exception has been raised while validating '%value%'" => "Произошла ошибка во время проверки значения номера кредитной карточки '%value%'",
    ...
    'Invalid type given. String, integer, array or Zend_Date expected' => 'Недопустимый тип данных. Значение должно быть строкой, целым числом, массивом или объектом Zend_Date',
    ...
    "File '%value%' is not readable or does not exist" => "Файл '%value%' не может быть прочитан или не существует",
    ...
    "'%value%' does not appear to be a valid URI hostname" => "Значение '%value%' является недопустимым URI имени хоста",
    ...
    'The input does not appear to be a postal code' => "'%value%' не является корректным почтовым кодом",

    // Zend_Validate_Regex
    'Invalid type given, value should be string, integer or float' => 'Недопустимый тип данных, значение должно быть числом с плавающей точкой, строкой, или целым числом',
    'Invalid type given. String, integer or float expected' => 'Недопустимый тип данных. Значение должно быть строкой, целым числом или числом с плавающей точкой',
    "'%value%' does not match against pattern '%pattern%'" => "'%value%' не соответствует шаблону '%pattern%'",
    "The input does not match against pattern '%pattern%'" => "'%value%' не соответствует шаблону '%pattern%'",
