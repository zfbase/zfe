...
trait ZFE_Model_AbstractRecord_Autocomplete
{
    ...
    protected function _autocompleteToArray(array $array)
    {
        foreach (static::$autocompleteCols as $key => $options) {
            ...
            $related = $this->get($alias);
            // с помощью clearRelated пытаемся предотвратить
            // создание новых пустых сущностей при сохранении
            if (!$related || !$related->exists()) {
                $this->clearRelated($alias);
                $related = null;
            }
            ...
            if ($related && $related instanceof ZFE_Model_AbstractRecord) {
                foreach ($related::$autocompleteSelectCols as $field) {
