...
class ZFE_Tasks_Manager
{
     ...
     */
    static public function getInstance(Zend_Config $cfg = null)
    {
     ...
     */
    public function findAllRevisionsFor(Tasks $task)
    {
    ...
    /**
     * Для данной записи найди задачу, которая
     * уже ранее была запланирована и еще не выполнена
     *
     ...
     * @throws ZFE_Tasks_Exception
     */
    public function assign(Tasks $task)
    {
    ...
    final public function manage(Doctrine_Collection_OnDemand $tasks)
    {
        ...
        foreach ($tasks as $task) {
            ...
            /** @var ZFE_Model_Default_Tasks $task */
            if ($task->getState() === static::STATE_DONE) continue;
     ...
     * @param DateTime|null $sheduleDateTime
     *
     * @return null|Tasks
     */
    public function plan(
        ...
        if ($sheduleDateTime) {
            $task->setScheduledAt($sheduleDateTime);
            $task->save();
        }
     ...
     *
     * Запланировать повторное выполение успешно выполненной задачи
     ...
     * @throws ZFE_Tasks_Exception
     */
    public function revision(Tasks $task, $force = false)
    {
     ...
     * @return Tasks
     */
    public function finish(Tasks $task)
    {
